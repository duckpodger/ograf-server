...
class RendererManager {
    ...
    async listRenderers(ctx) {
        ...
        ctx.body = (0, lib_1.literal)({ renderers });
    }
    /** A ServerAPI Method */
    async getRendererManifest(ctx) {
        ...
        ctx.status = 200;
        ctx.body = (0, lib_1.literal)(result);
    }
    async getRendererStatus(ctx) {
        ...
        if (!rendererInstance) {
            ctx.status = 404;
            ctx.body = (0, lib_1.literal)({ code: 404, message: `Renderer ${params.rendererId} not found` });
            return;
        }
        const status = {
        // TBD
